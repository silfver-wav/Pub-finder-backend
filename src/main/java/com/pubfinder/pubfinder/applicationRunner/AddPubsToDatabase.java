    ...
    private List<Pub> parseThroughArray(JSONArray jsonArray) throws JSONException {
        ...
        for (int i = 0; i < jsonArray.length(); i++) {
            ...
            if (obj.getJSONObject("additionalInfo").has("Service options")) {
                outDoorSeating = getOutDoorSeating(obj.getJSONObject("additionalInfo").getJSONArray("Service options"));
            }
            Accessibility accessibility;
            if (obj.getJSONObject("additionalInfo").has("Accessibility")) {
                accessibility = getAccessibility(obj.getJSONObject("additionalInfo").getJSONArray("Accessibility"));
            } else {
